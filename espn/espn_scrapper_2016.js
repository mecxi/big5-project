...
var server = 'http://192.168.8.250';
/* initialise common behaviours */
var scrapper_log = $('.display span:nth-child(2)');
...
/* start scrapping */
$('button:contains("Start Scrapper")').click(function(){
    /* clean up the scrapping logger */
    $('.display span:nth-child(2)').html('');
    /* get user input and start scrapping process */
    start_scrapping($('#url_name').val());
});

/* Print CSV */
$('button:contains("Print CSV")').click(function(){
    request_printing_report('CSV', $('#team_option option:selected').val());
});
...
function start_scrapping(url){
    ...
    $('#no_match_player').html('');
    $('#team_reference').html('<option value="0">Select Team</option>');
    $('#players_reference').html('<option value="0">Select a player</option>');
    ...
    for (var i=0; i < espn_urls.rugby.length; ++i){
        ...
        if (scrapper_year == '2016'){
            ...
            espn_scrapper_2017(espn_urls.rugby[i], i);
        } else {
...
function process_espn_result(espn_match){
    $.ajax({
...
function populate_option_team(){
    $(document).ready(function(){
        $('#team_option').html('<option value="all">Print Team Option</option>');
        /* populate team */
        $.ajax({
...
function no_match_options(obj_result){
    var no_match_box =  $('.no_match_area #no_match_player');
    var team = [];
    if (obj_result !== undefined){
        for(var i=0; i < obj_result.length; ++i){
            ...
            no_match_box.append(text_reference+'<br><button class="add_option" player_name="'+t[0].trim()+'">Add Reference</button><br>');
            //alert(t[1]);
            //team.push(t[1].trim());
    ...
    for (var j=0; j < team.length; ++j){
        $('#team_reference').append('<option value="'+team[j]+'">'+team[j]+'</option>');
    }
    /* initialise button event on click */
    $('button.add_option').on('click', function(){
        add_player_reference($(this).attr('player_name'), $(this));
    });
    //console.log(team);
}

/* populate players for a select team option */
$('#team_reference').on('change', function(){
    //alert($(this).find('option:selected').val());
    var selected_team =  $(this).find('option:selected').val();
    /* clear existing options */
    $('#players_reference').html('<option value="0">select a player to add reference</option>');
    $.ajax({
        url: server + '/big5/options.php',
        data: JSON.stringify({
            option: 'players',
        ...
        success: function(result){
            if (result.result == null || result.error){
                scrapper_log.append(current_timestamp()+ 'Enable to populate players option for team :'+selected_team+'<br>');
            } else {
                var players = result.result;
                for (var i=0; i < players.length; ++i){
                    $('#players_reference').append('<option value="'+players[i].id+'">'+players[i].firstname+' '+players[i].lastname+'</option>');
                }
            }
        },
        error:function(){
...
function add_player_reference(player_ref_btn, btn){
    ...
    var selected_player_id = $('#players_reference option:selected').val();
    if (selected_player_id == '0'){
        scrapper_log.append(current_timestamp()+ 'Please select a player from the lookup Reference Option<br>');
    } else {
        /* check player reference for accuracy */
        var sel_opt = $('#players_reference option:selected').html().split(' ');
        ...
        switch ($('[name="accuracy"]:checked').val()){
            case '0':
                ...
                if (sel_opt[0] == btn_opt[0]){
                    ...
                    scrapper_log.append(current_timestamp()+ '<i style="background-color: #4b0000">Error</i> updating reference - firstname referenced accuracy does not match. Please select accuracy option<br>');
                }
                break;
            case '1':
                /* lastname accuracy */
                if (sel_opt.slice(-1).pop() == btn_opt.slice(-1).pop()){
                    ...
                    scrapper_log.append(current_timestamp()+ '<i style="background-color: #4b0000">Error</i> updating reference - lastname referenced accuracy does not match. Please select option<br>');
                }
                break;
            case '2':
